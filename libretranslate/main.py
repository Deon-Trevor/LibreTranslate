...
def get_args():
    ...
    parser.add_argument(
        "--port", type=int, help="Port (%(default)s)", default=DEFARGS["PORT"]
    )
    parser.add_argument(
        "--char-limit",
    ...
    parser.add_argument(
        "--suggestions",
        default=DEFARGS["SUGGESTIONS"],
        action="store_true",
        help="Allow user suggestions",
    )
    parser.add_argument(
        "--disable-files-translation",
        default=DEFARGS["DISABLE_FILES_TRANSLATION"],
        action="store_true",
        help="Disable files translation",
    )
    parser.add_argument(
        "--disable-web-ui",
        default=DEFARGS["DISABLE_WEB_UI"],
        action="store_true",
        help="Disable web ui",
    )
    parser.add_argument(
        "--update-models",
        default=DEFARGS["UPDATE_MODELS"],
        action="store_true",
        help="Update language models at startup",
    )
    parser.add_argument(
        "--force-update-models",
        default=DEFARGS["FORCE_UPDATE_MODELS"],
        action="store_true",
        help="Install/Reinstall language models at startup",
    )
    parser.add_argument(
        "--metrics",
    ...
    parser.add_argument(
        ...
        type=str,
        help="Add a prefix like /url-prefix to URL: example.com:5050/url-prefix/",
    )
...
def main():
    ...
    if args.url_prefix:
        ...
        def redirect(environ, start_response):
            start_response(
                "301 REDIRECT",
                [("Content-Type", "text/plain"), ("Location", args.url_prefix)],
            )
            yield b"Redirect..."

        app = DispatcherMiddleware(redirect, {args.url_prefix: create_app(args)})
    else:
